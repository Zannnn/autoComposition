#这段代码构造出部分前景物体、全部可见遮罩和amodal遮罩，后面用pix2gestalt构造被遮挡物体的前景
import json
import os
import numpy as np
...
# 路径配置
json_path = "/pub/data/lz/autoComposition/autocomp_master/dataset/BSDSAandCOCOA/annotations/COCO_amodal_train2014.json"
image_root = "/pub/data/lz/autoComposition/autocomp_master/dataset/COCO2014/train2014/"

# output_filefold = "/pub/data/lz/autoComposition/autocomp_master/dataset/forComp_dataset_all/"
output_filefold = "/pub/data/lz/autoComposition/autocomp_master/dataset/forComp_COCO2014_dataset_50/"

output_foreground = output_filefold + "foreground"
output_vismask = output_filefold + "objmask_visiable"
output_amodal = output_filefold + "objmask_amodal"
...
# 打开文件用于写入保存的文件名
foreground_log = open(output_filefold + "foreground.txt", "w")
vismask_log = open(output_filefold + "vismask.txt", "w")
amodalmask_log = open(output_filefold + "amodalmask.txt", "w")
...
for annotation in tqdm(data["annotations"]):
    ...
    if not image_info:
        continue
    if os.path.basename(json_path) == "COCO_amodal_train2014.json":  #如果是COCO而不是BSDS
        image_id = f"COCO_train2014_{int(image_id):012d}"
    ...
    for idx, region in enumerate(annotation["regions"]):
        ...
        # 在外面保存 amodal mask，  里面分别保存visable mask
